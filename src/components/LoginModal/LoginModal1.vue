...
const handleLogin = async () => {
  ...
  try {
    ...
    const nickname = loginForm.value.phone || 'DemoUser'
    userStore.loginAction({ nickname })
...
const registerRules = {
  ...
  password: [
    ...
    { pattern: passwordReg, message: '密码长度为6-20位，支持字母、数字、特殊字符', trigger: 'blur' }
  ],
  confirmPassword: [
...
const handleRegister = async () => {
  ...
  try {
    ...
    const nickname = registerForm.value.username || registerForm.value.phone || 'NewUser'
    userStore.registerAction({ nickname })
