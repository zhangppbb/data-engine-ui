  ...
  const httpRequest = async (options: UploadRequestOptions) => {
    ...
    if (isClientUpload) {
      ...
      return new Promise((resolve, reject) => {
        const moduleCode = options.data as unknown as string
        const moduleMap = moduleStore.getModuleByCode(moduleCode)
        const param = {
          file: options.file,
          path: moduleMap.attachmentCatalogueCode,
          configId: moduleMap.fileConfigId,
          moduleCode
        }
        FileApi.updateFile(param)
          .then((res) => {
            if (res.code === 0) {
              resolve(res)
            } else {
