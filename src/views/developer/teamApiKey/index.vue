<template>
  <ContentWrap>
    <div class="flex justify-between items-center mb-4">
      <h2 class="text-lg font-semibold">团队 API Key</h2>
      <div class="flex items-center gap-4">
        <el-select v-model="selectedTeam" placeholder="选择团队">
          <el-option v-for="team in teamList" :key="team.id" :label="team.name" :value="team.id" />
        </el-select>
        <el-button type="primary" @click="openCreateModal" :disabled="!selectedTeam">
  ...
  <el-dialog title="创建团队 API Key" v-model="createModalVisible">
...
import type { TeamVO } from '@/api/developer/team'
...
const loadApiKeys = async () => {
  ...
  const res = await ApiKeyApi.listTeamApiKeys(selectedTeam.value)
