...
/** 子表的表单 */
const subTabsName = ref('demo03Grade')
...
const submitForm = async () => {
  ...
  formLoading.value = true
  try {
    const data = formData.value as unknown as Demo03StudentVO
    // 拼接子表的数据
    data.demo03GradeList = demo03GradeFormRef.value.getData()
    data.demo03CourseList = demo03CourseFormRef.value.getData()
    ...
    emit('success')
    // 刷新指定模块列表页
    refreshAppointPage('infra_demo03_student')
  } finally {
    formLoading.value = false
  }
}
