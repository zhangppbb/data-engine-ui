<template>
  ...
  <ContentWrap>
    <el-form
      ...
      label-width="68px"
    >
      <el-form-item label="数据源" prop="dataSourceConfigCode">
        <el-select
          v-model="queryParams.dataSourceConfigCode"
          class="!w-240px"
          clearable
          placeholder="请选择数据源"
        >
          <el-option v-for="name in dataSourceConfigList" :key="name" :label="name" :value="name" />
        </el-select>
      </el-form-item>
  ...
  <ContentWrap>
    <el-table v-loading="loading" :data="list">
      ...
      <el-table-column align="center" label="数据源" prop="dataSourceConfigCode" />
      <el-table-column
        label="生成模版"
        align="center"
        prop="templateType"
      ...
      >
        <template #default="scope">
          <dict-tag :type="DICT_TYPE.INFRA_CODEGEN_TEMPLATE_TYPE" :value="scope.row.templateType" />
        </template>
      </el-table-column>
      ...
      <el-table-column align="center" fixed="right" label="操作" width="300px">
        <template #default="scope">
          <el-button
            v-if="scope.row.templateType !== InfraCodegenTemplateTypeEnum.SUB"
...
const queryParams = reactive({
  pageNo: 1,
  pageSize: 10,
  dataSourceConfigCode: 'master',
  tableName: undefined,
  tableComment: undefined,
  createTime: [] as any
})
const queryFormRef = ref() // 搜索的表单
const dataSourceConfigList = ref<string[]>([]) // 数据源列表
