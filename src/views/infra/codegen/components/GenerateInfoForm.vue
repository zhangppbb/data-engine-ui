<template>
  <el-form ref="formRef" :model="formData" :rules="rules" label-width="150px">
    <el-row>
      ...
      <el-col :span="12">
        <el-form-item>
          ...
          <el-tree-select
            ...
            node-key="id"
            placeholder="请选择系统菜单"
            :disabled="formData.templateType === InfraCodegenTemplateTypeEnum.SUB"
          />
        </el-form-item>
      </el-col>
      ...
      <el-col :span="12">
        <el-form-item label="生成导出功能" prop="needExport">
          <el-radio-group style="--el-text-color-regular: #666;--el-fill-color-blank: #fff;" v-model="formData.needExport">
            <el-radio
      ...
      <el-col :span="12">
        <el-form-item label="生成导入功能" prop="needImport">
          <el-radio-group style="--el-text-color-regular: #666;--el-fill-color-blank: #fff;" v-model="formData.needImport">
            <el-radio
              v-for="dict in getBoolDictOptions(DICT_TYPE.INFRA_BOOLEAN_STRING)"
              :key="dict.value as string"
              :value="dict.value"
    ...
    <el-row v-if="formData.templateType == 2">
      ...
      <el-col :span="12">
        <el-form-item prop="treeNameRuleColumnId">
          <template #label>
            <span>
              树名称规则字段
              <el-tooltip content="树名称的规则字段名， 如：层级一/层级二/层级三/" placement="top">
                <Icon icon="ep:question-filled" />
              </el-tooltip>
            </span>
          </template>
          <el-select v-model="formData.treeNameRuleColumnId" placeholder="请选择">
            <el-option
              v-for="(column, index) in props.columns"
              :key="index"
              :label="column.columnName + '：' + column.columnComment"
      ...
      <el-col :span="12">
        <el-form-item prop="treeIdRuleColumnId">
          <template #label>
            <span>
              树主键规则字段
              <el-tooltip content="树主键的规则字段名， 如：1/2/3/" placement="top">
                <Icon icon="ep:question-filled" />
              </el-tooltip>
            </span>
          </template>
          <el-select v-model="formData.treeIdRuleColumnId" placeholder="请选择">
            <el-option
              v-for="(column, index) in props.columns"
              :key="index"
              :label="column.columnName + '：' + column.columnComment"
    ...
    <el-row v-if="formData.templateType == InfraCodegenTemplateTypeEnum.SUB">
      ...
      <el-col :span="12">
        <el-form-item prop="subJoinMany">
          ...
          <el-radio-group style="--el-text-color-regular: #666;--el-fill-color-blank: #fff;" v-model="formData.subJoinMany" placeholder="请选择">
...
const formData = ref({
  ...
  treeNameRuleColumnId: undefined,
  treeIdRuleColumnId: undefined,
  needExport: undefined,
  needImport: undefined
})
...
const rules = reactive({
  ...
  treeNameRuleColumnId: [required],
  treeIdRuleColumnId: [required],
  needExport: [required],
  needImport: [required]
})
...
watch(
  ...
  async (table) => {
    ...
    // 加载表列表
    if (table.dataSourceConfigCode != '') {
      tables.value = await CodegenApi.getCodegenTableList(formData.value.dataSourceConfigCode)
    }
  },
  {
    deep: true,
