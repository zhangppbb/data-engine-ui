...
/** 示例分类 表单 */
defineOptions({ name: 'Demo02CategoryForm' })
...
const submitForm = async () => {
  ...
  formLoading.value = true
  try {
    const data = formData.value as unknown as Demo02CategoryVO
    ...
    emit('success')
    // 刷新指定模块列表页
    refreshAppointPage('infra_demo02_category')
  } finally {
    formLoading.value = false
  }
}
...
const getDemo02CategoryTree = async () => {
  ...
  const data = await Demo02CategoryApi.getDemo02CategoryList(null)
