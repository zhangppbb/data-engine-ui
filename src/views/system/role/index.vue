<template>
      ...
      <el-table-column :width="400" align="center" label="操作">
        <template #default="scope">
          ...
          <el-button
            ...
            title="数据权限"
            type="primary"
            @click="openDataPermissionFormChain(scope.row)"
          >
            数据权限
          </el-button>
          <el-button
  ...
  <!-- 表单弹窗：数据权限（链） -->
  <RoleDataPermissionFormChain ref="dataPermissionFormChainRef" @success="getList" />
  <!-- 表单弹窗：分配用户 -->
  <RoleUserForm ref="userFormRef" @success="getList" />
</template>
...
import RoleUserForm from './RoleUser.vue'
import RoleDataPermissionFormChain from './RoleDataPermissionFormChain.vue'
...
/** 数据权限（链）操作 */
const dataPermissionFormChainRef = ref()
const openDataPermissionFormChain = async (row: RoleApi.RoleVO) => {
  dataPermissionFormChainRef.value.open(row)
}

/** 分配用户操作 */
const userFormRef = ref()
const openUser = (row: RoleApi.RoleVO) => {
...
const handleDelete = async (id: number) => {
  try {
    // 删除的二次确认
    await message.delConfirm("是否确认删除该角色？删除后所有绑定该角色的用户权限将全部失效，请您谨慎操作")
    // 发起删除
