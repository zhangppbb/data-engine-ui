  ...
  <Dialog v-model="dialogVisible" :title="dialogTitle">
    ...
    >
      ...
      <el-form-item
        label="上级字典项"
        prop="parentId"
        v-if="dictCategory === SystemDictCategoryEnum.CASCADE"
      >
        <el-tree-select
...
import { CommonStatusEnum, SystemDictCategoryEnum } from '@/utils/constants'
...
const formRules = reactive({
  ...
  parentId: [{ required: true, message: '上级字典项不能为空', trigger: 'change' }]
})
const formRef = ref() // 表单 Ref
const dictDataTree = ref() // 树形结构
const defaultProps = {
  children: 'children',
  label: 'label',
  value: 'id',
...
/** 打开弹窗 */
const open = async (type: string, category: number, dictType: string, id?: number) => {
  dialogVisible.value = true
  dialogTitle.value = t('action.' + type)
  dictCategory.value = category
  formType.value = type
  resetForm()
  if (dictType) {
  ...
  if (dictCategory.value === SystemDictCategoryEnum.CASCADE) {
    await getTree()
  }
}
...
const getTree = async () => {
  ...
  // 修改字典时，上级字典项不能选择自己和后代
  // 后端返回规则编码，模糊匹配判断
  if (formType.value === 'update') {
    data.forEach((dict) => {
      if (dict.labelRuleData.startsWith(formData.value.labelRuleData)) {
        dict.disabled = true
      }
    })
  }
  let dictData = { id: 0, label: '顶级字典项' }
  dictData['children'] = handleTree(data)
  dictDataTree.value.push(dictData)
}
