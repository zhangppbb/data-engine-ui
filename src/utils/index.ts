 ...
 * @return 格式化后的数量
 */
export const countInputFormatter = (num: number | string | undefined) => {
  return numberFormatter(num, COUNT_DIGIT)
}
 ...
 * @return 格式化后的数量
 */
export const countTableColumnFormatter = (_, __, cellValue: any, ___) => {
  return numberFormatter(cellValue, COUNT_DIGIT)
}
 ...
 * @return 格式化后的数量
 */
export const priceInputFormatter = (num: number | string | undefined) => {
  return numberFormatter(num, PRICE_DIGIT)
}
 ...
 */
export const priceMultiply = (price: number, count: number) => {
  if (price == null || count == null) {
    return undefined
  }
  return parseFloat((price * count).toFixed(PRICE_DIGIT))
}
...
/**
 * 在字符串数字后面补零，使其长度至少为指定的位数。
 * 如果原始字符串长度小于指定长度，则在末尾添加零补齐。
 * 如果长度已经达到或超过指定长度，则返回原字符串。
 *
 * @param {string} numStr - 要处理的字符串数字。
 * @param {number} [targetLength=6] - 目标字符串长度，默认为 6 位。
 * @returns {string} - 补零后的字符串数字。
 */
export const padZero = (numStr: string, targetLength: number = 6): string => {
  // 转换为字符串并检查长度是否小于目标长度
